...
import com.wowotoffer.shelf.auth.properties.ShelfValidateCodeProperties;
...
public class ValidateCodeServiceImpl implements ValidateCodeService {
    ...
    public void create(HttpServletRequest request, HttpServletResponse response) throws IOException, ValidateCodeException {
        ...
        ShelfValidateCodeProperties code = properties.getCode();
        setHeader(response, code.getType());

        // 生成Captcha
        Captcha captcha = createCaptcha(code);
        // 将验证码存入Redis
        redisService.set(ShelfConstant.CODE_PREFIX + key, StringUtils.lowerCase(captcha.text()),code.getTime());
        // 输出图片
    ...
    public void check(String key, String value) throws ValidateCodeException {
        Object codeInRedis = redisService.get(ShelfConstant.CODE_PREFIX + key);
    ...
    private Captcha createCaptcha(ShelfValidateCodeProperties code) {
